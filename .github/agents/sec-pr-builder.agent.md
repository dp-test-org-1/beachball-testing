...
* **For Transitive Dependencies:** If nested, inject a global override in the root `package.json` under the "pnpm.overrides" block using #tool:edit. **CRITICAL:** Use the vulnerable version range from the pnpm audit report output to define overrides and club if range overrides with each other from report. For each vulnerable version range, create an override entry (e.g., `"package-name@>1.0.0 <2.0.0": "^1.0.5"` and `"package-name@>2.0.2 <3.0.0": "^2.0.3"` as distinct entries). Use caret (^) version ranges for overrides to allow compatible minor and patch updates. Do NOT use versions without ^; always use ^ ranges to enable automatic security patches. 
